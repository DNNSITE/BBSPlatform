...
(function ($) {
    ...
    var stopEvnt = false, uploadFileOpenEditor = function (nm) {
    ...
    $.each(d, function (i, itm) {
        if (itm.type == 'extlist' && itm.name == panename) {
            var v = itm.value; if (v) {
                $('#frmVal_' + panename + settings.modid).val(v); $('#frmVal_' + panename + settings.modid).change(); $('#frmVal_' + panename +

        settings.modid).valid();
            } return;
        }
    });
},
    ...
    $.each(d, function (i, itm) {
        ...
        if (pObj.length) {
            ...
            if (itm.type == 'text') {
                ...
                $(cntrObj).on('change', function () { if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, $(this).val()); } if (!stopEvnt) { settings.onChange.call(this); if (itm.srchble) { settings.onChange_SrchbleFld.call(this); } else { settings.onChange_NotSrchbleFld.call(this); } } });
            }
            if (itm.type == 'extvalidtext') {
                ...
                html += '</label></div>';
                pObj.replaceWith(html);
                $(cntrObj).on('change', function () { extvalidtext_Valid(itm); if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, $(this).val()); } if (!stopEvnt) { settings.onChange.call(this); if (itm.srchble) { settings.onChange_SrchbleFld.call(this); } else { settings.onChange_NotSrchbleFld.call(this); } } });
            ...
            if (itm.type == 'multitext') {
                ...
                $(cntrObj).on('change', function () {
                    if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, $(this).val()); } settings.onChange.call(this);
            ...
            if (itm.type == 'rangeslider') {
                ...
                html += '</label></div>';
                pObj.replaceWith(html);
                $(cntrObj).ionRangeSlider(itm.options);
                $(cntrObj).on('change', function () {
                    if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, $(this).val()); }
            ...
            if (itm.type == 'rangedigit') {
                ...
                $((cntrObj + '_min,' + cntrObj + '_max')).on('change', function () {
                    ...
                    if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, dval); } settings.onChange.call(this); if (itm.srchble) { settings.onChange_SrchbleFld.call(this); } else { settings.onChange_NotSrchbleFld.call(this); }
                });
            }
            if (itm.type == 'email') {
                html = '<div id="ctrPane_' + itm.name + settings.modid + '" data-visible="' + defVsl + '" data-enable="' + defEnbl + '"' + (defVsl == false ? ' style="display:none;"' : '') + '>';
            ...
            if (itm.type == 'date') {
                ...
                html += '</label></div>';
                pObj.replaceWith(html);
                $(cntrObj).datepicker({ dateFormat: 'dd.mm.yy', prevText: '<i class="fa fa-chevron-left"></i>', nextText: '<i class="fa fa-chevron-right"></i>' });
            ...
            if (itm.type == 'money') {
                html = '<div id="ctrPane_' + itm.name + settings.modid + '" data-visible="' + defVsl + '" data-enable="' + defEnbl + '"' + (defVsl == false ? ' style="display:none;"' : '') + '>';
                var plcholder = '', strArrVal = '', defval_prc = '', defval_cur = '', inptmask = '# ##0';
                if (itm.hasOwnProperty('plcholder')) { plcholder = itm.plcholder; }
                ...
                html += '<div class="form-group">';
                html += '<label class="input' + (defEnbl == false ? ' state-disabled' : '') + (cntrReq ? ' reqInd' : '') + '">';
                html += '<input type="text" id="frmVal_' + itm.name + settings.modid + '_price" name="frmVal_' + itm.name + settings.modid + '_price"' + (ro ? ' readonly="readonly"' : '') + (defEnbl == false ? ' disabled="disabled"' : '') + (defval_prc ? ' value="' + defval_prc + '"' : '') + (plcholder ? ' placeholder="' + plcholder + '"' : '') + ' maxlength="15" style="border-right:none;"/>';
                html += '</label>';
                html += '</div>';
                html += '<div class="form-group">';
                html += '<label class="selectpicker">';
                html += '<select id="frmVal_' + itm.name + settings.modid + '_curid" name="frmVal_' + itm.name + settings.modid + '_curid"' + (itm.plcholder ? ' data-placeholder="' + itm.plcholder + '"' : 'data-placeholder=" "') + ' class="selectpicker form-control" data-size="7">';
                if (itm.hasOwnProperty('curncdata')) { var d1 = itm.curncdata; $.each(d1, function (i2, itm2) { html += '<option value="' + itm2.id + '"' + (defval_cur == itm2.id ? ' selected="selected"' : '') + (itm2.hasOwnProperty('icncss') ? ' data-icon="' + itm2.icncss + '"' : '') + '>' + itm2.title + '</option>'; }); }
                else { html += '<option class="plcholder" value="">' + (itm.plcholder ? itm.plcholder : '') + '</option>'; }                
                html += '</select>';
                html += '</label>';
                html += '</div>';
                html += '<input type="hidden" id="frmVal_' + itm.name + settings.modid + '" name="frmVal_' + itm.name + settings.modid + '"' + (itm.defval ? ' value="' + itm.defval + '"' : '') + '>';
                ...
                html += '</div>';
                pObj.replaceWith(html);
                $(cntrObj + '_price').mask(inptmask, {reverse:true});
                if (strArrVal != '') { $(cntrObj).val(itm.defval); }
                $(cntrObj + '_price,' + cntrObj + '_curid').on('change', function () {
                    var mnyval = $(cntrObj + '_price').val(), mnycurval = $(cntrObj + '_curid').val(); mnyval = mnyval.replace(/ /g, ''); mnycurval = mnycurval.replace(/ /g, '');
                    if (isNaN(mnyval) == true) { $(cntrObj + '_price').val(''); mnyval = ''; } if (isNaN(mnycurval) == true) { $(cntrObj + '_curid').val(''); mnycurval = ''; }
                    var dval = (mnyval == '' ? 'null' : mnyval) + '|' + (mnycurval == '' ? 'null' : mnycurval); if (dval == 'null:null') { dval = ''; } $(cntrObj).val(dval); /*$(cntrObj).valid();*/
                    if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, dval); } settings.onChange.call(this); if (itm.srchble) { settings.onChange_SrchbleFld.call(this); } else { settings.onChange_NotSrchbleFld.call(this); }
                });
            }
            ...
            if (itm.type == 'extlist') {
                ...
                html += '</label></div>';
                pObj.replaceWith(html);
                if (itm.listautoload) { setControlListData(itm.name, itm.listsrcname, null); } else { cntrEnbl = false; }
                $(cntrObj).on('change', function () {
                    if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, $(this).val()); }
                    if (!stopEvnt) { settings.onChange.call(this); if (itm.srchble) { settings.onChange_SrchbleFld.call(this); } else { settings.onChange_NotSrchbleFld.call(this); } }
                });
            ...
            if (itm.type == 'selectpicker') {
                ...
                html += '</label></div>';
                pObj.replaceWith(html);
                if (!itm.hasOwnProperty('listdata') && itm.hasOwnProperty('listsrcurl') && itm.hasOwnProperty('listsrcname')) {
                    if (itm.hasOwnProperty('listautoload')) { if (itm.listautoload) { setControlListData(itm.name, itm.listsrcname, null); } else { cntrEnbl = false; } }
                }
                $(cntrObj).on('change', function () {                    
                    if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, $(this).val()); }
            ...
            if (itm.type == 'advlist') {
                ...
                html += '</label></div>';
                pObj.replaceWith(html);
                $(cntrObj).ajaxChosen({ dataType: 'json', type: 'POST', url: '' }, { loadingImg: '/images/loading16.gif', minLength: 2, generateUrl: function (q) { return getUrlAdvList(itm.name); } }, { width: "100%", no_results_text: 'Не найдено из: ', allow_single_deselect: true });
                $(cntrObj).on('change', function () {
                    $(this).valid(); if (itm.hasOwnProperty('triggers')) { triggerFldProc(itm.triggers, $(this).val()); }
                    if (!stopEvnt) { settings.onChange.call(this); if (itm.srchble) { settings.onChange_SrchbleFld.call(this); } else { settings.onChange_NotSrchbleFld.call(this); } }
                });
            ...
            setControlEnable(itm.name, cntrEnbl);
            if (itm.hasOwnProperty('validrule')) {
                setControlValidrule(itm, itm.validrule);
                //if (itm.validrule.length != 0) { $(cntrObj).rules('remove'); $(cntrObj).rules('add', itm.validrule); }
            }
        }
    });
    fillForm(settings.dataForm);
},
mixerDataForm = function (sdItm, fdItm) {
    var res = fdItm;
    if (!sdItm || typeof sdItm != 'object') { return res; }
    if (res.name != sdItm.name) { return res; }
    if (sdItm.hasOwnProperty('type')) { if (isCompatDataForm(sdItm.type, res.type) == false) { return res; } }
    $.each(sdItm, function (key, val) { res[key] = val; });
    return res;
},
isCompatDataForm = function (sdTp, fdTp) {
    var res = false;
    if (fdTp == 'text' || fdTp == 'phone' || fdTp == 'email' || fdTp == 'extvalidtext' || fdTp == 'multitext' || fdTp == 'rangedigit' || fdTp == 'rangeslider') {
        if (sdTp == 'text' || sdTp == 'phone' || sdTp == 'email' || sdTp == 'extvalidtext' || sdTp == 'multitext' || sdTp == 'rangedigit' || sdTp == 'rangeslider' || sdTp == 'selectpicker') { res = true; }
    }
    ...
    setControlListData = function (nm, pval) {
        ...
        $.each(d, function (i, itm) {
            if (itm.name == nm) {
                if (itm.type == 'extlist' || itm.type == 'selectpicker') {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (!isexitdata) { setControlEnable(itm.name, false); } else {
                                $(o).html('<option class="plcholder" value="">' + ($(o).attr('data-placeholder') ? $(o).data('placeholder') : '') + '</option>'); $.map(data, function (item) { $(o).append('<option value="' + item.id + '"' + (item.hasOwnProperty('icncss') ? ' data-icon="' + item.icncss + '"' : '') + '>' + item.title + '</option>'); });
                                setControlEnable(itm.name, true);
                            }
    ...
    setControlValidrule = function (itm, v) {        
        if (itm == null || typeof itm != 'object') {return; }
        var vExts=0,cntrObj='';
        if (v != null && typeof v == 'object' && v.length != 0) { vExts = 1; }
        if (itm.type == 'text' || itm.type == 'money1' || itm.type == 'list' || itm.type == 'extlist' || itm.type == 'selectpicker') {
        ...
        if (itm.type == 'money') {            
            cntrObj = '#frmVal_' + itm.name + settings.modid + '_price';            
            $(cntrObj).rules('remove');
            if (vExts == 1) {
                $(cntrObj).rules('add', v);
            }
    ...
    fillForm = function (df) {
        var isClr = false, cntrObj = '', itmval;
        if (df == null || typeof df != 'object') { isClr = true; } else if (df.length == 0) { isClr = true; }
        if (isClr) { df = settings.dataForm; } stopEvnt = true;
    ...
    getFormData = function (isRaw) {
        ...
        $.each(df, function (i, itm) {
            v = '';
            if (itm.type == 'text' || itm.type == 'multitext' || itm.type == 'rangeslider' || itm.type == 'email' || itm.type == 'phone' || itm.type == 'date' || itm.type == 'list' || itm.type == 'extlist' || itm.type == 'selectpicker' || itm.type == 'money') {
