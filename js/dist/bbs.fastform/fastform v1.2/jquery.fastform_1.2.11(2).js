    ...
    $.each(d, function (i, itm) {
        ...
        if (pObj.length) {
            ...
            if (itm.type == 'chek') {
                ...
                //if (!settings.hideTitle) { html += '<label class="label' + (cntrReq ? ' reqInd' : '') + '" for="frmVal_' + itm.name + settings.modId + '">' + itm.title + '</label>'; }
                html+='<label class="label'+(cntrReq ? ' reqInd' : '')+'" for="frmVal_'+itm.name+settings.modId+'">&nbsp;</label>';
            ...
            if (itm.name == nm) {
                if (itm.type == 'extlist' || itm.type == 'selectpicker') {
                    ...
                    $.ajax({
                        ...
                        data: f,
                        success: function (data) {
                            var isexitdata=true/*,oVal=$(o).data('val')*/;if(!data||typeof data!='object'){isexitdata=false;}if(data.length==0){isexitdata=false;}
                            ...
                            setControlVal(itm.name,$(o).data('val'));                            
                            if(itm.type=='selectpicker'){$(o).selectpicker('refresh');}
                        }
                    });
                }
                return false;
