    ...
    $.each(d, function (i, itm) {
        ...
        if (pObj.length) {
            ...
            if (itm.type == 'money') {
                ...
                if (itm.hasOwnProperty('curncdata')) { $.each(itm.curncdata, function (i2, itm2) { html += '<option value="' + itm2.id + '"' + (defval_cur == itm2.id ? ' selected="selected"' : '') + (itm2.hasOwnProperty('icncss') ? ' data-icon="' + itm2.icncss + '"' : '') + '>' + itm2.title + '</option>'; }); }
                else if (typeof settings.currencyData == 'object') {
                    $.each(settings.currencyData, function (i2, itm2) { html += '<option value="' + itm2.id + '"' + (defval_cur == itm2.id ? ' selected="selected"' : '') + (itm2.hasOwnProperty('icncss') ? ' data-icon="' + itm2.icncss + '"' : '') + '>' + itm2.title + '</option>'; });
                } else {
            ...
            if (itm.type == 'ageman') {
                ...
                html += '</label></div>';
                pObj.replaceWith(html);
                $.fn.editableform.buttons = '<button type="submit" class="btn btn-success editable-submit btn-mini"><i class="fa fa-check"></i></button>' + '<button type="button" class="btn editable-cancel btn-mini"><i class="fa fa-times"></i></button>';
...
isCompatFormData = function (sdTp, fdTp) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (!isexitdata) { setControlEnable(itm.name, false); } else {
                                if (itm.type == 'selectpicker') { if (data.length >= 10) { $(o).attr('data-live-search', "true"); } else { $(o).removeAttr('data-live-search'); } }
    ...
    setControlValidrule = function (itm) {
    ...
    getFormData = function (isRaw) {
        ...
        $.each(df, function (i, itm) {
            v = '';
            if (itm.type == 'text' || itm.type == 'multitext' || itm.type == 'email' || itm.type == 'phone' || itm.type == 'date' || itm.type == 'list' || itm.type == 'extlist' || itm.type == 'selectpicker' || itm.type == 'money') {
            ...
            if (itm.type == 'ageman') {
                v = $('#frmVal_' + itm.name + settings.modId).agemaneditor('getValue').replace(/,/g, '|');
            }
            if (itm.type == 'attachfiles') {
