...
(function($){
    ...
    ,initWidget=function($this,ro){
        var settings=getSettings($this),d=settings.formData,html=$($this).html(),wdgId=$($this).attr('id');
        if(settings.spinnerEnable){
            if($('#'+wdgId+'_spin').length){$($this).before('<div id="'+wdgId+'_spin" class="text-center"><i class="fa fa-circle-o-notch fa-spin"></i></div>');}
            $($this).hide();
        }        
        if(!d||typeof d!='object'){return;}if(d.length==0){return;}
        ...
        $.each(d,function(i,itm){
            var pObj=$('#fldPane_'+itm.name+settings.modId),cntrObj='#frmVal_'+itm.name+settings.modId,defVsl=true,defEnbl=true,cntrEnbl=true,cntrReq=false,lftaddon="",rgtaddon="",help="",html='';
            if(pObj.length){
                ...
                if (itm.hasOwnProperty('lftaddon')){lftaddon=itm.lftaddon}
                if (itm.hasOwnProperty('rgtaddon')){rgtaddon=itm.rgtaddon;}
                if (itm.hasOwnProperty('help')){help=itm.help;}               

                if (itm.ctype=='text'){
                    html = '<div id="ctrPane_' + itm.name + settings.modId + '" data-visible="' + defVsl + '" data-enable="' + defEnbl + '"' + (defVsl == false ? ' style="display:none;"' : '') + ' class="form-group">';
                    if (!settings.hideTitle) { html += '<label class="' + (defEnbl == false ? ' state-disabled' : '') + (cntrReq ? ' reqInd' : '') + '" for="frmVal_' + itm.name + settings.modId + '">' + itm.title + '</label>'; }                    
                    html+='<div class="'+(lftaddon!=''||rgtaddon!=''?' input-group':'')+(defEnbl==false?' state-disabled':'')+(cntrReq?' reqInd':'')+'">';
                    if(lftaddon!=''){html+='<p class="input-group-addon">'+itm.lftaddon+'</p>';}
                    ...
                    if(rgtaddon!=''){html+='<p class="input-group-addon">'+itm.rgtaddon+'</p>';}
                    if(itm.hasOwnProperty('tooltip')){if(itm.tooltip){html+='<b class="tooltip tooltip-top-right">'+itm.tooltip+'</b>';}}
                    if(itm.hasOwnProperty('help')){if(itm.help){html+='<p class="help-block">'+itm.help+'</p>';}}
                    html += '</div></div>';
                ...
                if (itm.ctype=='multitext'){
                    html = '<div id="ctrPane_' + itm.name + settings.modId + '" data-visible="' + defVsl + '" data-enable="' + defEnbl + '"' + (defVsl == false ? ' style="display:none;"' : '') + ' class="form-group">';
                    if (!settings.hideTitle) { html += '<label class="' + (cntrReq ? ' reqInd' : '') + '" for="frmVal_' + itm.name + settings.modId + '">' + itm.title + '</label>'; }
                ...
                if (itm.ctype=='email'){
                    html = '<div id="ctrPane_' + itm.name + settings.modId + '" data-visible="' + defVsl + '" data-enable="' + defEnbl + '"' + (defVsl == false ? ' class="form-group" style="display:none;"' : '') + ' class="form-group">';
                ...
                if(itm.ctype=='date'){
                    ...
                    html = '<div id="ctrPane_' + itm.name + settings.modId + '" data-visible="' + defVsl + '" data-enable="' + defEnbl + '"' + (defVsl == false ? ' class="form-group" style="display:none;"' : '') + '>';
                    if (!settings.hideTitle) { html += '<label class="label' + (cntrReq ? ' reqInd' : '') + '" for="frmVal_' + itm.name + settings.modId + '">' + itm.title + '</label>'; }
                    html += '<label class="input' + (cntrReq ? ' reqInd' : '') + '">';
                ...
                if(itm.ctype=='mdeditor'){
                    console.log(itm.name,itm.ctype);
                    console.log("htmltempl ",itm.htmltempl);
                    console.log("-----");
                    html='<div id="ctrPane_'+itm.name+settings.modId+'" data-visible="'+defVsl+'" data-enable="'+defEnbl+'"'+(defVsl==false?' style="display:none;"':'')+'>';
                    ...
                    $('#ctrPane_'+itm.name+settings.modId).bbsMDEditor({
                        ...
                        ,dVal:null
                        ,modId:'mdItemsPane_'+itm.name+settings.modId
                        ,listSrcUrl:settings.listSrcUrl
                        ,listSrcApiKey:settings.listSrcApiKey
                        ,HTMLTemplate:itm.htmltempl
                        /*,disabled:false
                        ,onChange:function(){}*/
                    });
                }
    ...
    ,setControlListData=function($this,nm,pval){
        var settings=getSettings($this),d=settings.formData,$itm;
        if(!d||typeof d!='object'){return;}if(d.length==0){return;}        
        $.each(d,function(i,itm){if(itm.name==nm&&(itm.ctype=='extlist'||itm.ctype=='advlist'||itm.ctype=='select2'||itm.ctype=='selectpicker')){$itm=itm;return false;}});
        if($itm==null){return;}
        var o='#frmVal_'+$itm.name+settings.modId;                
    ...
    ,getMDSelectorData=function($this,nm){
        ...
        $.each(d,function(i,itm){if(itm.name==nm&&itm.ctype=='mdselector'){$itm=itm;return false;}});
        if($itm==null){return;}        
 ...
 ,getSelect2TempResult=function(repo){
    ...
    ,getFormData=function($this,validIgrore){
        ...
        $.each(df,function(i,itm){
            ...
            if(itm.ctype=='text'||itm.ctype=='multitext'||itm.ctype=='email'||itm.ctype=='phone'||itm.ctype=='date'||itm.ctype=='list'||itm.ctype=='extlist'||itm.ctype=='selectpicker'||itm.ctype=='accountselector'||itm.ctype=='mdselector'||itm.ctype=='xeditor_mdfolderselect'||itm.ctype=='xeditor_locationselect'){
                v=$(cntrObj).val();
            }
            if(itm.ctype=='rangeslider'){
    ...
    ,convertToFormData=function($this,d){
        ...
        if(df==null||typeof df!='object'){return;}if(df.length==0){return null;}        
        var dn=[],val;
        if(d[0]==null||typeof d[0]!='object'){
            $.each(d,function(k,v){
                $.each(df,function(i,itm){if(k==itm.name){dn.push({"name":itm.name,"val":v,"type":itm.type,"ctype":itm.ctype}); return true;}});
            });            
        }else{
            $.each(d,function(i1,itm1){
                ...
                $.each(df,function(i2,itm2){if(itm1.name==itm2.name){dn.push({"name":itm2.name,"val":val,"type":itm2.type,"ctype":itm2.ctype});return false;}});
            });
        }
        return dn;
    }
