    ...
    $.each(d, function (i, itm) {
        ...
        if (pObj.length) {
            ...
            if (itm.type == 'advlist') {
                ...
                html += '</label></div>';
                pObj.replaceWith(html);
                $(cntrObj).ajaxChosen({ dataType: 'json', type: 'POST', url: '' }, { loadingImg: '/images/loading16.gif', minLength: 2, generateUrl: function (q) { return getUrlAdvList(itm.name); } }, { width: "100%", no_results_text: '�� ������� ��: ', allow_single_deselect: true });
                $(cntrObj).on('change', function () {
...
isCompatFormData = function (sdTp, fdTp) {
    ...
    getFormData = function (isRaw) {
        ...
        $.each(df, function (i, itm) {
            ...
            if (itm.type == 'ageman') {
                v = $('#frmVal_' + itm.name + settings.modId).agemaneditor('getValue');/*console.log('VVVVVV: ',v);*/
