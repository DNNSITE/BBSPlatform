...
(function($){
        ...
        $.each(d,function(i,itm){
            ...
            if(pObj.length){
                ...
                if (itm.type == 'extlist') {
                    ...
                    html += '</label></div>';
                    pObj.replaceWith(html);
                    if (itm.listautoload) { setControlListData($this,itm.name, itm.listsrcname, null); } else { cntrEnbl = false; }
                    $(cntrObj).on('change', function () {
                        if (itm.hasOwnProperty('triggers')){triggerFldProc($this,itm.triggers,$(this).val());}
                        if (!stopEvnt) { settings.onChange.call(this); if (itm.srchble) { settings.onChange_SrchbleFld.call(this); } else { settings.onChange_NotSrchbleFld.call(this); } }
                    });
                ...
                if (itm.type == 'selectpicker') {
                    ...
                    if (!itm.hasOwnProperty('listdata') && itm.hasOwnProperty('listsrcurl') && itm.hasOwnProperty('listsrcname')) {
                        if (itm.hasOwnProperty('listautoload')){if(itm.listautoload){setControlListData($this,itm.name,itm.listsrcname,null);}else{cntrEnbl=false;}}
                    }
                    $(cntrObj).on('change', function () {
                        if (itm.hasOwnProperty('triggers')){triggerFldProc($this,itm.triggers,$(this).val());}
    ...
    ,getFormData=function($this,isRaw){
        ...
        $.each(df, function (i, itm) {
            v = '';
            if(itm.type=='text'||itm.type=='multitext'||itm.type=='email'||itm.type=='phone'||itm.type=='date'||itm.type=='list'||itm.type=='extlist'||itm.type=='selectpicker'){
