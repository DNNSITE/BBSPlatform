...
(function($){    
    var defaults={
        ...
        ,getValue:function(){var v='';this.each(function(){v=getValue(this);return false;});return v;}                        
    };
    ...
    ,initWidget=function($this){
        ...
        $.each(settings.currListData,function(i,itm){
            html+='<option value="'+itm.id+'"'+(dVal_cur==itm.id?' selected="selected"':'')+(itm.hasOwnProperty('icncss')?' data-icn="'+itm.icncss+'"':'')+'>'+itm.title+'</option>';
        });        
        html+='</select>';
        html+='</div>';
        html+='</div>';        
    ...
    $.fn.bbsMoneyInput=function(methodOrOptions){        
        if(methods[methodOrOptions]){
            ...
            $.error('Method '+methodOrOptions+' does not exist on jQuery.bbsMoneyInput');
        }
    };
})(jQuery);