...
(function ($) {
    ...
    var paneId,initWidget=function(){
        ...
        if(settings.organizationSelectEnable==true&&settings.anonymousPersonSelectEnable==false){
            $('#frmDealerAccountGUID'+settings.modId).select2({
                ...
                ,templateResult:getOrganizationResultTemp
                ,templateSelection:getOrganizationSelectionTemp 
            });        
            $('#frmDealerContactUsers'+settings.modId).select2({
                ...
                ,templateResult:getOrganizationUserResultTemp
                ,templateSelection:getOrganizationUserSelectionTemp   
            });
        ...
        if(settings.personSelectEnable==true&&settings.anonymousPersonSelectEnable==false){
            $('#frmUserAccountGUID'+settings.modId).select2({
                ...
                ,minimumResultsForSearch:settings.minimumResultsForSearch
                ,placeholder:settings.localzRes.personFldSelectorPlaceholderText
                ,templateResult:getFriendResultTemp
                ,templateSelection:getFriendSelectionTemp 
            });        
            loadFriendList();
        }
    },
    loadOrganizationList=function(){        
        ...
        /*var data=[{"id":"7f0fe6fc-7c90-4fe8-a7c8-34ec1508a443","title":"Базовый электрод ООО","logourl":"assets/img/8.png","subtitle":"Россия, Москва, 23КМ МКАД, д.1"},{"id":"7f0fe6fc-7c90-4fe8-a7c8-34ec1508a444","title":"Красный лесоруб ООО","logourl":"assets/img/2.png","subtitle":"Россия, Черноморск, Кривоколенный пер., д.123"}];*/
    ...
    loadOrganizationUserList=function(accid){        
        var o='#frmDealerContactUsers'+settings.modId;$(o).html(''),f={"srcname":"D854896F-B5C2-4A9C-93E4-4E149D72AE7E","accid":accid},dVal=$(o).data('val'),isSel=false;
        if(!accid||accid==''){$(o).prop("disabled",true);return;}        
        /*var data=[{"id":"1","title":"Иванов Иван Иванович","logourl":"assets/img/1.png","subtitle":"Директор"},{"id":"2","title":"Петров Виталий Федорович","logourl":"assets/img/2.png","subtitle":"Бухгалтер"},{"id":"3","title":"Сидоров Степан Петрович","logourl":"assets/img/3.png","subtitle":"Завхоз"},{"id":"4","title":"Ладыженский Евгений Николаевич","logourl":"assets/img/4.png","subtitle":"Заместитель бухгалтера"}];*/
        $(o).prop("disabled",false);
        $.ajax({
            url:settings.listSrcUrl+'&jsoncallback=?',
            dataType:'json',
    ...
    getOrganizationResultTemp=function(s){        
    ...
    getOrganizationSelectionTemp=function(s){        
        var d=$(s.element).data('meta');
    ...
    getOrganizationUserResultTemp=function(s){        
        if (!s.id) {return s.text;}        
        var d=$(s.element).data('meta');
        var m='<div class="select2-result-repository">' +
                    '<div class="select2-result-repository__avatar"><img src="' + d.logourl + '" class="img-sm img-circle"/></div>' +
    ...
    getOrganizationUserSelectionTemp=function(s){
        if (!s.id) {return s.text;}
        var d=$(s.element).data('meta');
        var m='<div class="select2-result-repository">' +
                    '<div class="select2-result-repository__avatar"><img src="' + d.logourl + '" class="img-sm img-circle"/></div>' +
                    ...
                    '<div class="select2-result-repository__title">' + d.title + '</div>';
        if(d.subtitle){m+="<div class='select2-result-repository__subtitle'>" + d.subtitle + "</div>";}        
        /*m+='<div class="select2-result-repository__tbar">' +
                    '<a href="#" class="btn btn-icon fa fa-commenting-o icon-lg" alt="'+'Send message'+'" title="'+'Send message'+'"></a>' +                    
                '</div>' +
            '</div></div>';*/
        m+='</div>';        
        return m;
    },
    getFriendResultTemp=function(s){        
        if (!s.id) {return s.text;}        
        var d=$(s.element).data('meta');
        var m='<div class="select2-result-repository">' +
                    '<div class="select2-result-repository__avatar"><img src="' + d.logourl + '" class="img-sm img-circle"/></div>' +
    ...
    getFriendSelectionTemp=function(s){
        if (!s.id) {return s.text;}
        var d=$(s.element).data('meta');
        var m='<div class="select2-result-repository">' +
                    '<div class="select2-result-repository__avatar"><img src="' + d.logourl + '" class="img-sm img-circle"/></div>' +
